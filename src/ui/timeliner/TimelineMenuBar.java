...
// references to external objects
 ...
 // private static Logger log = Logger.getLogger(TimelineMenuBar.class);
  ...
  JMenuItem menuiNudgeLeft;
  JMenuItem menuiNudgeRight;
  ...
  private void createTimelineMenu() {
    ...
    menuiNudgeLeft = new JMenuItem("Nudge Left");
    menuiNudgeLeft.setFont(timelineMenuFont);
    menuiNudgeRight = new JMenuItem("Nudge Right");
    menuiNudgeRight.setFont(timelineMenuFont);
    ...
    menuTimepoint.add(menuiNudgeLeft);
    menuTimepoint.add(menuiNudgeRight);
    ...
    if (System.getProperty("os.name").startsWith("Mac OS")) {
      ...
      menuiNudgeLeft.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, InputEvent.CTRL_DOWN_MASK));
      menuiNudgeRight.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, InputEvent.CTRL_DOWN_MASK));      
    ...
    menuiNudgeLeft.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          if (pnlTimeline.getTimeline().areTimepointsSelected()) {
            // NUDGE LEFT!
        	 int tNum = pnlTimeline.getTimeline().getLastTimepointClicked();
        	 pnlTimeline.nudgeTimepointLeft(tNum);
            uilogger.log(UIEventType.MENUITEM_SELECTED, "nudge timepoint left" + pnlTimeline.getTimeline().getLastTimepointClicked());
          }
          else {
            // NUDGE MARKER LEFT!
            uilogger.log(UIEventType.MENUITEM_SELECTED, "nudge marker left" + pnlTimeline.getTimeline().getLastMarkerClicked());
          }
        }
      });
    menuiNudgeRight.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          if (pnlTimeline.getTimeline().areTimepointsSelected()) {
            // NUDGE RIGHT!
         	 int tNum = pnlTimeline.getTimeline().getLastTimepointClicked();
         	 pnlTimeline.nudgeTimepointRight(tNum);
        	  uilogger.log(UIEventType.MENUITEM_SELECTED, "nudge  right" + pnlTimeline.getTimeline().getLastTimepointClicked());
          }
          else {
            // NUDGE MARKER RIGHT!
        	  uilogger.log(UIEventType.MENUITEM_SELECTED, "nudge marker right" + pnlTimeline.getTimeline().getLastMarkerClicked());
          }
        }
      });
  ...
  protected void disableSelectedTimepointOptions() {
    ...
    menuiNudgeRight.setEnabled(false);
    menuiNudgeLeft.setEnabled(false);
  }
  ...
  protected void enableSelectedTimepointOptions() {
    ...
    menuiNudgeRight.setEnabled(true);
    menuiNudgeLeft.setEnabled(true);
  }
