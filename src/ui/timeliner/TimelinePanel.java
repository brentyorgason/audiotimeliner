...
// associated objects
  ...
  /**
   * nudgeTimepointLeft: nudges the specified timepoint left
   */
  public void nudgeTimepointLeft(int tNum) {
    if (timeline != null && timeline.isEditable()) {	
    	timeline.makeDirty();
      ...
      timeline.sortedPixelList[tNum] = timeline.sortedPixelList[tNum]-1;
      Timepoint t = timeline.getTimepoint(tNum);
      draggingTimepoint = true;
      paintContext = DRAGGING_TIMEPOINT;
      
      int currOff = (int)((((float)timeline.sortedPixelList[tNum]) / (float) timeline.getLineLength()) * (float)timeline.getPlayerDuration());
      timeline.sortedPointList[tNum] = currOff;
     
      //timeline.sortList();
      paintDraggingTimepoint();
      t.showTime(true);
      t.select();
        ...
        uilogger.log(UIEventType.ITEM_DRAGGED, "timepoint nudged: " + tNum);

         // int newOffset = timeline.getOffsetAt(tNum);
          undoManager.undoableEditHappened(new UndoableEditEvent(this,
             new UndoableDragTimepoint(tNum, dragStartOffset, timeline.sortedPixelList[tNum], this)));
          updateUndoMenu();
  ...
  /**
   * nudgeTimepointRight: nudges the specified timepoint right
   */
  public void nudgeTimepointRight(int tNum) {
    if (timeline != null && timeline.isEditable()) {	
    	timeline.makeDirty();
    	if (timeline.playerIsPlaying()) {
    		wasPlaying= true;
