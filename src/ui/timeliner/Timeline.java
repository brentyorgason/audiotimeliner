...
// timeline properties
  ...
  public int sortedPointList[] = new int[1000];  // arbitrary large number of points
  ...
  // media length
  public int mediaLength = 100000;  // default value here, later set to duration of audio
  ...
  protected void updatePopupMenus() {
    ...
    for (int i = 0; i < selectedBubbles.size(); i ++) {
      ...
      try {
    	  ...
    	  log.debug("Update popup menus error");
      }
 ...
 */
  ...
  protected void dragTimepoint (Point p) {
    ...
    if (lastTimepointClicked != 0) {
      ...
      if ((currMouseX > prevX + pixelBuffer) && (currMouseX < (nextX - pixelBuffer + 2))) {
        ...
        // that is, the mouse has moved substantially below or above the timeline
        // for it to be considered intentional removal (30 pixels currently)
        if (currMouseY < -30 || currMouseY > 30) {
  ...
  protected String getPlayerContent() {
    if (pnlTimeline.getFrame().isUsingLocalAudio) {
      ...
      // revert from downloaded media to online media 
      String originalPath = tLocalPlayer.originalMedia;
